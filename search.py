...
from thumos_features import my_collate_fn
...
def optimal_sequence_search(net, config, logger, train_loader):
    with torch.no_grad():
        ...
        for i in range(len(train_loader.dataset)):
            _index, _data, _label, _point_anno, _, _, _, proposal_bbox, proposal_count_by_video, pseudo_instance_label, dynamic_segment_weights_cumsum = next(loader_iter)

            _data = _data.to(torch.device('cuda:1'))
            _label = _label.to(torch.device('cuda:1'))
            _point_anno = _point_anno.to(torch.device('cuda:1'))
            
            _, cas_sigmoid_fuse, _, _, _ = net(_data, vid_labels=_label, proposal_bbox=proposal_bbox, proposal_count_by_video=proposal_count_by_video)
    ...
    for c in range(act_seed.shape[1]):
        ...
        if act_idx[0] > 0:
            if bkg_seed_c[:act_idx[0]].sum() > 0:
                ...
                # transcript记录动作与背景的分布情况  动作为1 背景为0
                transcript = [0, 1]
            else:
