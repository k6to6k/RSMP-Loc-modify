...
def test(net, config, logger, test_loader, test_info, step, step_iter, model_file=None):
    with torch.no_grad():
        net.eval()
        ...
        for i in range(len(test_loader.dataset)):
            ...
            vid_score, cas_sigmoid_fuse, _, _, _ = net(_data, vid_labels=_label, proposal_bbox=proposal_bbox, proposal_count_by_video=proposal_count_by_video)
        ...
        json_path = os.path.join(config.output_path, 'temp_result_step{}.json'.format(step_iter))
        ...
        test_info["iter"].append(step_iter)
